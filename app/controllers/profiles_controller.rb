class ProfilesController < ApplicationController
    ...
    if current_user.id == @user.id
      ...
      if current_user.subscriptions.exists?(friend_id: @user.id)
        ...
        if @subscription.save
          redirect_to profile_path(@user), notice: "Вы успешно подписались на данного пользователя"
        end
  ...
  def unsubscribe
    if current_user.id == @user.id
            redirect_to profile_path(@user), notice: "Вы не можете быть подписаны на самого себя"
    else
      if current_user.subscriptions.exists?(friend_id: @user.id)
